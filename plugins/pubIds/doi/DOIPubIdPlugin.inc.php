...
class DOIPubIdPlugin extends PubIdPlugin {
	...
	public function register($category, $path, $mainContextId = null) {
		...
		if ($success && $this->getEnabled($mainContextId)) {
			...
			//CUL Customization: trigger DOI assignment on publish hook 
			HookRegistry::register('Publication::publish', array($this, 'assignDOI'), HOOK_SEQUENCE_NORMAL);
		}
		return $success;
	}

	//CUL Customization: creates an assigns DOI
public function assignDOI($hookName, $args) {
	$pubObj = $args[0];
        $pubid = $this->getPubId($pubObj);
        $publicationDao = DAORegistry::getDAO('PublicationDAO');
        $publicationDao->changePubId($pubObj->getId(), $this->getPubIdType(), $pubid);
}
